...
public class ScriptContextCallGUIFilter extends DefaultGUIFilter {
	...
	public List<VordelPage> getPropertyPages() {
		...
		pages.add(new ScriptContextCallPage());
...
//	}
	...
	private void updateInitializerCircuit(Entity entity) {
		...
		if (initializerPK != null) {
			...
			if (parentPK.equals(initializerPK)) {
				Trace.fatal(String.format("Configuration error the bound call '%s' is pointing to it's parent circuit", filter.getName()));
	...
	private void updateChildEntity(EntityStore es, Entity child) {
		...
		if (childType.equals(inputType) || childType.equals(outputType)) {
